...
function drawGrid() {
    ...
    // Grid steps in meters (1mm and 10mm)
    const fineStep = 0.001;   
    const coarseStep = 0.01;  

    // Determine the world coordinates of the viewport corners
    const xMin = -offsetX / scale;
    const xMax = (w - offsetX) / scale;
    const yMin = (offsetY - h) / scale; // Screen Y is inverted
    const yMax = offsetY / scale;

    /* ---------- 1. Fine Grid (1mm) ---------- */
    // Use a color that is visible but subtle (Light gray/white with low alpha)
    ctx.strokeStyle = "rgba(100, 116, 139, 0.2)"; 
    ctx.lineWidth = 0.5;

    let xStart = Math.floor(xMin / fineStep) * fineStep;
    let yStart = Math.floor(yMin / fineStep) * fineStep;

    ctx.beginPath();
    for (let x = xStart; x <= xMax; x += fineStep) {
        const sx = x * scale + offsetX;
        ctx.moveTo(sx, 0);
        ctx.lineTo(sx, h);
    }
    for (let y = yStart; y <= yMax; y += fineStep) {
        const sy = -y * scale + offsetY; // Note the negative y for Cartesian
        ctx.moveTo(0, sy);
        ctx.lineTo(w, sy);
    }
    ctx.stroke();
    ...
    xStart = Math.floor(xMin / coarseStep) * coarseStep;
    yStart = Math.floor(yMin / coarseStep) * coarseStep;

    ctx.beginPath();
    for (let x = xStart; x <= xMax; x += coarseStep) {
        const sx = x * scale + offsetX;
        ctx.moveTo(sx, 0);
        ctx.lineTo(sx, h);
    }
    for (let y = yStart; y <= yMax; y += coarseStep) {
    ...
    /* ---------- 3. Origin Axes ---------- */
    // Bright color for the main crosshair
...
function draw() {
    ...
    if (points.length > 0) {
        ...
        let first = worldToScreen(points[0].x, points[0].y);
        ctx.moveTo(first.x, first.y);

        for (let i = 1; i < points.length; i++) {
            let p = worldToScreen(points[i].x, points[i].y);
            ctx.lineTo(p.x, p.y);
        }

        if (tempPoint) {
            let tp = worldToScreen(tempPoint.x, tempPoint.y);
    ...
    dimensions.forEach(drawDimension);
    calculateProperties();
}

/* ================= DIMENSION ================= */
function drawDimension(dim) {
    const p1 = worldToScreen(dim.p1.x, dim.p1.y);
    const p2 = worldToScreen(dim.p2.x, dim.p2.y);
    ...
    ctx.beginPath();
    ctx.moveTo(p1.x, p1.y);
    ctx.lineTo(p1.x + arrowSize * Math.cos(angle - Math.PI / 6),
    ...
    ctx.fillText((length * 1000).toFixed(1) + ' mm', 0, 0);
...
function calculateProperties() {
    if (points.length < 3) {
        ['areaVal','cxVal','cyVal','perimVal','ixxVal','iyyVal']
            .forEach(id => document.getElementById(id).textContent = '0.0000');
        return;
    }

    let area = 0, cx = 0, cy = 0, perim = 0;
    ...
    document.getElementById('areaVal').textContent = area.toFixed(4);
    document.getElementById('cxVal').textContent = cx.toFixed(4);
    document.getElementById('cyVal').textContent = cy.toFixed(4);
    document.getElementById('perimVal').textContent = perim.toFixed(4);
    document.getElementById('ixxVal').textContent = ixx.toFixed(4);
    document.getElementById('iyyVal').textContent = iyy.toFixed(4);
}

/* ================= TOOL HANDLING ================= */
function setTool(tool, btn = null) {
    currentTool = tool;
    dimStart = null;
    shapeStart = null;
    tempPoint = null;
...
canvas.addEventListener('mousemove', e => {
    ...
    const p = snapPoint(screenToWorld(e.clientX - r.left, e.clientY - r.top));

    document.getElementById('coordX').textContent = (p.x * 1000).toFixed(1);
    document.getElementById('coordY').textContent = (p.y * 1000).toFixed(1);
...
canvas.addEventListener('click', e => {
    if (isPanning || currentTool === 'select') return;
    ...
    if (currentTool === 'line' || currentTool === 'polyline') {
        ...
        if (!shapeStart) shapeStart = p;
        else {
            points.push(
                {x:shapeStart.x,y:shapeStart.y},
                {x:p.x,y:shapeStart.y},
                {x:p.x,y:p.y},
                {x:shapeStart.x,y:p.y},
                {x:shapeStart.x,y:shapeStart.y}
            );
            shapeStart = null;
        }
        ...
        if (!shapeStart) shapeStart = p;
            ...
            for (let i = 0; i <= 64; i++) {
                ...
                points.push({
                    x: shapeStart.x + r * Math.cos(a),
                    y: shapeStart.y + r * Math.sin(a)
                });
            }
            shapeStart = null;
        }
